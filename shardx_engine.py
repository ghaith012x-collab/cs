    ...
    async def _handle(self, reader, writer):
        try:
            ...
            if is_connect:
                ...
                # readline here hangs the renderer forever, which shows up as
                # title="(unknown)" url="(unknown)" in the worker. On timeout
                # we close both ends so Chromium commits a clean proxy error
                # page and the worker rotates instead of wedging.
                try:
...
# Every session gets a random, plausible desktop-user timezone. The bundled
# fingerprint library is host-locale homogeneous (every profile carries the
# sandbox's own tz, e.g. Europe/Warsaw) — a fixed tz on a worldwide proxy
# pool is both a static fingerprint AND inconsistent with most exit IPs.
# Randomize so no two sessions ever share a timezone, matching the "always
# randomize fingerprints and everything" requirement. (An explicit caller
# timezone still wins — e.g. when the bot knows the proxy's region.)
_TZ_POOL = [
    "America/New_York", "America/Chicago", "America/Denver",
    "America/Los_Angeles", "America/Toronto", "America/Mexico_City",
    "America/Sao_Paulo", "Europe/London", "Europe/Dublin",
    "Europe/Berlin", "Europe/Paris", "Europe/Amsterdam",
    "Europe/Madrid", "Europe/Rome", "Europe/Stockholm",
    "Europe/Zurich", "Europe/Vienna", "Africa/Cairo",
...
def _build_profile(sdk: ShardX, kwargs: dict):
    ...
    tz = kwargs.get("timezone") or random.choice(_TZ_POOL)
    cfg["timezone"] = tz
    if isinstance(cfg.get("navigator"), dict):
        cfg["navigator"].setdefault("timezone", tz)
    return profile
        ...
        def _spawn():
            ...
            with _LAUNCH_LOCK:
                ...
                session = sdk.launch(
                    ...
                    extra_args=extra,
                    screen_mode="profile",
                    # Always re-randomize hardware / platform version at
                    # launch: fresh profile + fresh random device from the
                    # library + this = a new, unlinkable identity every
