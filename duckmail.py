"""
duckmail.py — fast temp-mail REST client (duckmail.sbs primary, mail.tm fallback).

Pure-HTTP replacement for the old Playwright incognitomail.co client.
Verified against the live APIs (2026-08):

  duckmail.sbs (api.duckmail.sbs)
    POST /accounts                 create mailbox           -> 201 account JSON
    POST /token                    address+password -> JWT  -> 200 {"token": ...}
    GET  /messages                 list inbox (Bearer)     -> {"hydra:member": [...]}
    GET  /messages/{id}            single message body

  mail.tm fallback (api.mail.tm) — same API-Platform/Hydra envelope:
    GET  /domains                  valid domains
    POST /accounts                 create mailbox
    POST /token
    GET  /messages

Both providers return the Hydra envelope, so a single parser serves both.

Discord signup flow:
  1. create_inbox()                  -> random addr@duckmail.sbs
...
MAILTM_DOMAIN = "duckmail.sbs"  # fallback domain hint (overridden by /domains)
...
class TempMail:
    """Async temp-mail client: duckmail.sbs primary, mail.tm fallback.
    ...
    def __init__(self, log: Optional[Callable] = None):
        ...
        self._provider = ""        # 'duckmail' | 'mailtm'
        self._address = ""
        self._password = ""
        self._token = ""
    ...
    async def create_inbox(self, timeout: float = 40.0) -> str:
        ...
        Tries duckmail.sbs first (fast, purpose-built), then mail.tm.
        """
    ...
    async def _create_on(self, base: str, provider: str,
                         timeout: float) -> bool:
        """Try to create + authorize a mailbox on one provider."""
        deadline = time.time() + timeout
        domain = DUCKMAIL_DOMAIN = "duckmail.sbs"
        if provider == "mailtm":
