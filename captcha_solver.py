...
# ── Objects / function ──
    ...
    # ── Food sources / animal products (very common in the field) ──
    (r"animal.*gives us steak|gives us steak|steak.*from.*animal|animal.*steak", "cow"),
    (r"animal.*(?:provides|gives).*milk|milk.*(?:comes|come).*from", "cow"),
    (r"animal.*(?:produces|give).*wool|wool.*from.*(?:sheep|animal)", "sheep"),
    (r"animal.*lays? eggs|eggs.*(?:come|comes).*from", "hen"),
    (r"animal.*gives us eggs|eggs.*(?:lay|laying).*animal", "hen"),
    (r"animal.*(?:bacon|ham)|pork.*(?:comes|come).*from", "pig"),
    (r"animal.*gives us bacon|bacon.*(?:comes|come).*from.*pig", "pig"),
    (r"animal.*(?:produces|make).*honey|honey.*(?:come|comes).*from.*bee", "bee"),
    (r"animal.*gives us honey|honey.*made by", "bee"),
    (r"animal.*(?:wool|fleece)", "sheep"),
    (r"meat.*(?:comes|come).*from.*cow|beef.*(?:comes|come).*from", "cow"),
    (r"(?:what|which) meat.*cow|beef", "beef"),
    (r"(?:what|which) animal.*(?:lamb|mutton)", "sheep"),
    (r"chicken.*meat.*(?:comes|come)|poultry", "chicken"),
    # ── Camping / outdoor tools (field-failure #2) ──
    (r"use.*dig.*camping|dig while camping|camping.*dig", "shovel"),
    (r"(?:what|which).*use.*(?:camping.*tent|sleep.*camping|tent.*camping)", "tent"),
    (r"(?:what|which).*use.*camping.*(?:cook|food)|cook.*camping", "camp stove"),
    (r"(?:what|which).*use.*(?:start.*fire|make.*fire)", "matches"),
    (r"(?:what|which).*use.*(?:light.*fire|fireplace)", "matches"),
    (r"(?:what|which).*use.*(?:cut.*wood|chop.*wood|wood.*axe)", "axe"),
    (r"(?:what|which).*use.*(?:cut.*tree|saw.*wood|wood.*saw)", "saw"),
    (r"(?:what|which).*use.*(?:see.*dark.*night|dark.*night.*see)", "flashlight"),
    (r"(?:what|which).*use.*(?:camping.*sleep|sleep.*(?:bag|outside))", "sleeping bag"),
    (r"(?:what|which).*use.*(?:carry.*water|drink.*water.*camping)", "canteen"),
    (r"(?:what|which).*use.*(?:swim|lake|pool)", "swimsuit"),
    (r"(?:what|which).*use.*(?:hike|walk.*trail|trek)", "hiking boots"),
    (r"(?:what|which).*use.*(?:protect.*head.*(?:hard|construction))", "helmet"),
    # ── Everyday objects / tools by function ──
    (r"(?:what|which).*use.*(?:screw|tighten.*screw)", "screwdriver"),
    (r"(?:what|which).*use.*(?:cut.*cloth|fabric)", "scissors"),
    (r"(?:what|which).*use.*(?:cut.*hair)", "scissors"),
    (r"(?:what|which).*use.*(?:measure.*(?:length|distance))", "ruler"),
    (r"(?:what|which).*use.*(?:weigh|weight|scale)", "scale"),
    (r"(?:what|which).*use.*(?:bake|cake|oven)", "oven"),
    (r"(?:what|which).*use.*(?:fry|cook.*pan)", "pan"),
    (r"(?:what|which).*use.*(?:boil.*(?:water|egg)|pot)", "pot"),
    (r"(?:what|which).*use.*(?:eat.*(?:cereal|soup)|soup)", "spoon"),
    (r"(?:what|which).*use.*(?:cut.*(?:food|meat|bread|steak))", "knife"),
    (r"(?:what|which).*use.*(?:drink.*(?:coffee|tea|water)|cup)", "cup"),
    (r"(?:what|which).*use.*(?:brush.*(?:hair|teeth)|toothbrush)", "toothbrush"),
    (r"(?:what|which).*use.*(?:dry.*(?:hands|body)|towel)", "towel"),
    (r"(?:what|which).*use.*(?:clean.*(?:floor|house)|mop)", "mop"),
    (r"(?:what|which).*use.*(?:sweep|broom)", "broom"),
    (r"(?:what|which).*use.*(?:glue|stick.*(?:paper|things))", "glue"),
    (r"(?:what|which).*use.*(?:write.*(?:board|chalk))", "chalk"),
    (r"(?:what|which).*use.*(?:camera|take.*picture|photo)", "camera"),
    (r"(?:what|which).*use.*(?:phone|call)", "phone"),
    (r"(?:what|which).*use.*(?:keyboard|type)", "keyboard"),
    (r"(?:what|which).*use.*(?:listen.*(?:music|audio))", "headphones"),
    # ── Science / nature / body ──
    (r"color of the sky|sky.*color", "blue"),
    (r"color of grass|grass.*color", "green"),
    (r"color of (?:the )?sun|sun.*color", "yellow"),
    (r"color of (?:the )?moon", "white"),
    (r"plants need.*(?:grow)|what do plants", "water"),
    (r"water.*(?:freezes|frozen)|freezing point.*water", "0"),
    (r"water.*(?:boils|boiling)", "100"),
    (r"gas.*(?:breathe|we breathe)|we breathe", "oxygen"),
    (r"honey.*(?:made|produced).*by", "bees"),
    (r"what (?:do )?bees make|bee makes", "honey"),
    (r"what do hens lay|hen lays", "eggs"),
    (r"what (?:do )?cows give|cow gives", "milk"),
    (r"what (?:do )?sheep give|sheep gives", "wool"),
    (r"what (?:do )?pigs give|pig gives", "pork"),
    (r"largest animal.*(?:land|earth)", "elephant"),
    (r"largest animal.*(?:ocean|sea|water)", "whale"),
    (r"largest animal", "blue whale"),
    (r"fastest land animal", "cheetah"),
    (r"tallest animal", "giraffe"),
    (r"animal.*(?:long neck|tallest)", "giraffe"),
    (r"animal.*(?:trunk)", "elephant"),
    (r"animal.*(?:stripes.*(?:black|white)|zebra)", "zebra"),
    (r"animal.*(?:spots.*(?:leopard|cheetah))", "leopard"),
    (r"king of (?:the )?jungle", "lion"),
    (r"animal.*(?:man.s|mans) best friend", "dog"),
    (r"animal.*(?:cat|meow)", "cat"),
    (r"insect.*(?:honey|pollinate)", "bee"),
    (r"insect.*(?:spin.*web|web)", "spider"),
    (r"insect.*(?:fly|wings.*colorful)", "butterfly"),
    (r"insect.*(?:stinger|yellow.*black)", "wasp"),
    # ── Geography / general facts ──
    (r"largest (?:continent|landmass)", "asia"),
    (r"smallest (?:continent|landmass)", "australia"),
    (r"largest ocean", "pacific"),
    (r"largest planet", "jupiter"),
    (r"closest planet.*(?:sun|earth)", "mercury"),
    (r"red planet", "mars"),
    (r"planet.*(?:rings)", "saturn"),
    (r"planet.*(?:we live|earth)", "earth"),
    (r"hottest planet", "venus"),
    (r"largest country.*(?:area|land)", "russia"),
    (r"most populous country", "india"),
    (r"longest river", "nile"),
    (r"highest mountain", "everest"),
    (r"largest desert", "sahara"),
    (r"coldest continent", "antarctica"),
    (r"how many (?:continents|countries).*world", "7"),
    (r"how many oceans", "5"),
    (r"how many (?:days|day).*week", "7"),
    (r"how many (?:months).*year", "12"),
    (r"how many (?:hours).*day", "24"),
    (r"how many (?:minutes).*hour", "60"),
    (r"how many (?:seconds).*minute", "60"),
    (r"how many (?:planets)", "8"),
    (r"how many colors.*rainbow", "7"),
    (r"how many sides.*triangle", "3"),
    (r"how many sides.*square", "4"),
    (r"how many sides.*(?:pentagon|star)", "5"),
    (r"how many sides.*hexagon", "6"),
    (r"how many (?:wheels).*car", "4"),
    (r"how many (?:wheels).*(?:bicycle|bike)", "2"),
    (r"how many legs.*(?:dog|cat|horse|cow)", "4"),
    (r"how many legs.*spider", "8"),
    (r"how many legs.*(?:insect|ant|bee)", "6"),
    (r"how many legs.*(?:bird|chicken|human|person)", "2"),
    (r"how many (?:eyes|ears)", "2"),
    (r"how many fingers.*hand", "5"),
    (r"how many toes.*foot", "5"),
    # ── First/last/order facts ──
    (r"first month.*year", "january"),
    (r"last month.*year", "december"),
    (r"first day.*week", "sunday"),
    (r"last day.*week", "saturday"),
    (r"month.*after june", "july"),
    (r"month.*after july", "august"),
    (r"month.*after august", "september"),
    (r"month.*after september", "october"),
    (r"month.*before january", "december"),
    (r"season.*after winter", "spring"),
    (r"season.*after spring", "summer"),
    (r"season.*after summer", "autumn"),
    (r"season.*after (?:autumn|fall)", "winter"),
    # ── Opposites (common variants) ──
    (r"opposite of (?:up|above)", "down"),
    (r"opposite of (?:down|below)", "up"),
    (r"opposite of hot", "cold"),
    (r"opposite of cold", "hot"),
    (r"opposite of (?:day|light)", "night"),
    (r"opposite of dark", "light"),
    (r"opposite of left", "right"),
    (r"opposite of right", "left"),
    (r"opposite of (?:big|large)", "small"),
    (r"opposite of (?:small|little)", "big"),
    (r"opposite of (?:open|unlocked)", "closed"),
    (r"opposite of fast", "slow"),
    (r"opposite of wet", "dry"),
    (r"opposite of (?:full|empty)", "empty"),
    (r"opposite of (?:old|young)", "young"),
    (r"opposite of happy", "sad"),
    (r"opposite of sad", "happy"),
    (r"opposite of (?:tall|short)", "short"),
    (r"opposite of (?:front|back)", "back"),
    (r"opposite of (?:early|late)", "late"),
    (r"opposite of (?:strong|weak)", "weak"),
    (r"opposite of (?:loud|quiet)", "quiet"),
    (r"opposite of (?:clean|dirty)", "dirty"),
    (r"opposite of (?:soft|hard)", "hard"),
    (r"opposite of (?:sweet|sour)", "sour"),
    (r"opposite of (?:buy|sell)", "sell"),
    (r"opposite of (?:enter|exit)", "exit"),
    (r"opposite of (?:push|pull)", "pull"),
    (r"opposite of (?:win|lose)", "lose"),
    (r"opposite of (?:begin|end)", "end"),
    (r"opposite of (?:start|finish)", "finish"),
    (r"opposite of (?:find|lose)", "lose"),
    (r"opposite of (?:give|take)", "take"),
    (r"opposite of (?:bring|take)", "take"),
    (r"opposite of (?:stand|sit)", "sit"),
    (r"opposite of (?:wake|sleep)", "sleep"),
    (r"opposite of (?:question|answer)", "answer"),
    (r"opposite of (?:north|south)", "south"),
    (r"opposite of (?:east|west)", "west"),
    (r"opposite of (?:narrow|wide)", "wide"),
    (r"opposite of (?:thick|thin)", "thin"),
    (r"opposite of (?:deep|shallow)", "shallow"),
    (r"opposite of (?:high|low)", "low"),
    (r"opposite of (?:heavy|light)", "light"),
    (r"opposite of (?:sharp|dull)", "dull"),
    (r"opposite of (?:smooth|rough)", "rough"),
    (r"opposite of (?:kind|mean)", "mean"),
    (r"opposite of (?:polite|rude)", "rude"),
    (r"opposite of (?:brave|cowardly)", "cowardly"),
    (r"opposite of (?:rich|poor)", "poor"),
    (r"opposite of (?:new|old)", "old"),
    (r"opposite of (?:modern|ancient)", "ancient"),
    (r"opposite of (?:true|false)", "false"),
    (r"opposite of (?:right|wrong)", "wrong"),
    (r"opposite of (?:correct|incorrect)", "incorrect"),
    (r"opposite of (?:success|failure)", "failure"),
    (r"opposite of (?:victory|defeat)", "defeat"),
    (r"opposite of (?:love|hate)", "hate"),
    (r"opposite of (?:friend|enemy)", "enemy"),
    (r"opposite of (?:peace|war)", "war"),
    (r"opposite of (?:war|peace)", "peace"),
    (r"opposite of (?:life|death)", "death"),
    (r"opposite of (?:birth|death)", "death"),
    (r"opposite of (?:sunrise|sunset)", "sunset"),
    (r"opposite of (?:morning|evening)", "evening"),
    (r"opposite of (?:winter|summer)", "summer"),
    (r"opposite of (?:summer|winter)", "winter"),
    (r"opposite of (?:spring|fall)", "fall"),
    (r"opposite of (?:rain|shine)", "shine"),
    (r"opposite of (?:beautiful|ugly)", "ugly"),
    (r"opposite of (?:pretty|ugly)", "ugly"),
    (r"opposite of (?:cheap|expensive)", "expensive"),
    (r"opposite of (?:simple|complex)", "complex"),
    (r"opposite of (?:safe|dangerous)", "dangerous"),
    (r"opposite of (?:possible|impossible)", "impossible"),
    (r"opposite of (?:first|last)", "last"),
    (r"opposite of (?:last|first)", "first"),
    (r"opposite of (?:inside|outside)", "outside"),
    (r"opposite of (?:include|exclude)", "exclude"),
    (r"opposite of (?:accept|reject)", "reject"),
    (r"opposite of (?:pass|fail)", "fail"),
    (r"opposite of (?:stop|go)", "go"),
    (r"opposite of (?:forward|backward)", "backward"),
    (r"opposite of (?:above|below)", "below"),
    (r"opposite of (?:increase|decrease)", "decrease"),
    (r"opposite of (?:positive|negative)", "negative"),
    (r"opposite of (?:winning|losing)", "losing"),
    (r"opposite of (?:arrive|depart)", "depart"),
    (r"opposite of (?:import|export)", "export"),
    (r"opposite of (?:connect|disconnect)", "disconnect"),
    (r"opposite of (?:expand|contract)", "contract"),
    (r"opposite of (?:ascend|descend)", "descend"),
    (r"opposite of (?:ascending|descending)", "descending"),
    (r"opposite of (?:minimum|maximum)", "maximum"),
    (r"opposite of (?:maximum|minimum)", "minimum"),
    (r"opposite of (?:city|countryside)", "countryside"),
    (r"opposite of (?:interior|exterior)", "exterior"),
    (r"opposite of (?:internal|external)", "external"),
    (r"opposite of (?:democracy|dictatorship)", "dictatorship"),
    (r"opposite of (?:amateur|professional)", "professional"),
    (r"opposite of (?:ancient|modern)", "modern"),
    (r"opposite of (?:artificial|natural)", "natural"),
    (r"opposite of (?:calm|agitated)", "agitated"),
    (r"opposite of (?:cheerful|gloomy)", "gloomy"),
    (r"opposite of (?:clear|cloudy)", "cloudy"),
    (r"opposite of (?:cloudy|clear)", "clear"),
    (r"opposite of (?:coarse|fine)", "fine"),
    (r"opposite of (?:constant|variable)", "variable"),
    (r"opposite of (?:cruel|kind)", "kind"),
    (r"opposite of (?:courageous|cowardly)", "cowardly"),
    (r"opposite of (?:danger|safety)", "safety"),
    (r"opposite of (?:definite|indefinite)", "indefinite"),
    (r"opposite of (?:difficult|easy)", "easy"),
    (r"opposite of (?:easy|difficult)", "difficult"),
    (r"opposite of (?:early|late)", "late"),
    (r"opposite of (?:east|west)", "west"),
    (r"opposite of (?:elegant|plain)", "plain"),
    (r"opposite of (?:empty|full)", "full"),
    (r"opposite of (?:expand|shrink)", "shrink"),
    (r"opposite of (?:explicit|implicit)", "implicit"),
    (r"opposite of (?:false|true)", "true"),
    (r"opposite of (?:fast|slow)", "slow"),
    (r"opposite of (?:fat|thin)", "thin"),
    (r"opposite of (?:female|male)", "male"),
    (r"opposite of (?:friendly|hostile)", "hostile"),
    (r"opposite of (?:generous|stingy)", "stingy"),
    (r"opposite of (?:gentle|rough)", "rough"),
    (r"opposite of (?:guilty|innocent)", "innocent"),
    (r"opposite of (?:hard|soft)", "soft"),
    (r"opposite of (?:healthy|sick)", "sick"),
    (r"opposite of (?:honest|dishonest)", "dishonest"),
    (r"opposite of (?:horizontal|vertical)", "vertical"),
    (r"opposite of (?:hot|cold)", "cold"),
    (r"opposite of (?:hungry|full)", "full"),
    (r"opposite of (?:import|export)", "export"),
    (r"opposite of (?:inner|outer)", "outer"),
    (r"opposite of (?:intelligent|stupid)", "stupid"),
    (r"opposite of (?:interesting|boring)", "boring"),
    (r"opposite of (?:joy|sorrow)", "sorrow"),
    (r"opposite of (?:land|sea)", "sea"),
    (r"opposite of (?:lazy|active)", "active"),
    (r"opposite of (?:light|dark)", "dark"),
    (r"opposite of (?:loose|tight)", "tight"),
    (r"opposite of (?:loud|soft)", "soft"),
    (r"opposite of (?:low|high)", "high"),
    (r"opposite of (?:many|few)", "few"),
    (r"opposite of (?:married|single)", "single"),
    (r"opposite of (?:mountain|valley)", "valley"),
    (r"opposite of (?:noise|silence)", "silence"),
    (r"opposite of (?:north|south)", "south"),
    (r"opposite of (?:obey|disobey)", "disobey"),
    (r"opposite of (?:optimist|pessimist)", "pessimist"),
    (r"opposite of (?:ordinary|extraordinary)", "extraordinary"),
    (r"opposite of (?:patient|impatient)", "impatient"),
    (r"opposite of (?:polite|impolite)", "impolite"),
    (r"opposite of (?:private|public)", "public"),
    (r"opposite of (?:public|private)", "private"),
    (r"opposite of (?:pure|impure)", "impure"),
    (r"opposite of (?:quick|slow)", "slow"),
    (r"opposite of (?:quiet|loud)", "loud"),
    (r"opposite of (?:real|fake)", "fake"),
    (r"opposite of (?:regular|irregular)", "irregular"),
    (r"opposite of (?:relevant|irrelevant)", "irrelevant"),
    (r"opposite of (?:responsible|irresponsible)", "irresponsible"),
    (r"opposite of (?:rough|smooth)", "smooth"),
    (r"opposite of (?:rural|urban)", "urban"),
    (r"opposite of (?:satisfied|dissatisfied)", "dissatisfied"),
    (r"opposite of (?:secure|insecure)", "insecure"),
    (r"opposite of (?:selfish|selfless)", "selfless"),
    (r"opposite of (?:senior|junior)", "junior"),
    (r"opposite of (?:shallow|deep)", "deep"),
    (r"opposite of (?:silent|noisy)", "noisy"),
    (r"opposite of (?:south|north)", "north"),
    (r"opposite of (?:specific|general)", "general"),
    (r"opposite of (?:static|dynamic)", "dynamic"),
    (r"opposite of (?:strength|weakness)", "weakness"),
    (r"opposite of (?:subjective|objective)", "objective"),
    (r"opposite of (?:success|failure)", "failure"),
    (r"opposite of (?:summer|winter)", "winter"),
    (r"opposite of (?:sunny|cloudy)", "cloudy"),
    (r"opposite of (?:superior|inferior)", "inferior"),
    (r"opposite of (?:temporary|permanent)", "permanent"),
    (r"opposite of (?:thick|thin)", "thin"),
    (r"opposite of (?:tight|loose)", "loose"),
    (r"opposite of (?:top|bottom)", "bottom"),
    (r"opposite of (?:tough|tender)", "tender"),
    (r"opposite of (?:tragedy|comedy)", "comedy"),
    (r"opposite of (?:urban|rural)", "rural"),
    (r"opposite of (?:valid|invalid)", "invalid"),
    (r"opposite of (?:valuable|worthless)", "worthless"),
    (r"opposite of (?:victory|defeat)", "defeat"),
    (r"opposite of (?:visible|invisible)", "invisible"),
    (r"opposite of (?:war|peace)", "peace"),
    (r"opposite of (?:wet|dry)", "dry"),
    (r"opposite of (?:wide|narrow)", "narrow"),
    (r"opposite of (?:wise|foolish)", "foolish"),
    (r"opposite of (?:work|rest)", "rest"),
    (r"opposite of (?:young|old)", "old"),
...
async def solve_hcaptcha_accessibility(page, iframe, 
    ...
    async def _read_question_text() -> str:
        ...
        # CODE / JSON / HASHES / URLS must never be scored as questions.
        # Field case: hCaptcha frame-3 exposed a JS comment
        #   /* { "version": "1", "hash": "MEQCIFJ19q8BZ6Jf3YkQ/..." } */
        # that scored 4 (the "8/4" inside the base64) and was "solved" as
        # math 2 -- a wasted question typed into a real challenge.
        _JUNK_RE = re.compile(
            r'/\*|\*/|\b(?:const|let|var|function|return|import|export)\b|'
            r'"(?:version|hash|signature|payload)"|\{\s*"|https?://|wss?://|'
            r'=>|\\u[0-9a-fA-F]{4}|[A-Za-z0-9+/]{40,}={0,2}', re.IGNORECASE)
