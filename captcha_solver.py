...
class DragSolver(CaptchaSolver):
    ...
    async def _get_target_position_clip(self, frame, page: Page) -> Optional[Tuple[float, float]]:
        """
        Use CLIP to find the target drop position.
        Uses a small 3x3 grid (9 cells) for speed on CPU.
        CLIP model is a singleton - loads once, stays in memory.
        """
        try:
            print("DragSolver: Loading CLIP model (cached after first load)...")
            clip_model = await self._get_clip_model()
            print("DragSolver: CLIP ready, analyzing challenge...")
            ...
            print(f"DragSolver: Image size: {w}x{h}")
            
            # Use a 3x3 grid = 9 cells (fast on CPU)
            grid_cols = 3
            grid_rows = 3
            cell_w = w / grid_cols
            cell_h = h / grid_rows
            ...
            # Score each cell - look for the empty/outlined target area
            target_prompts = [
                "an empty outlined area where an icon should be placed",
                "a dashed outline or placeholder slot",
                "a target drop zone with an outline",
                "an empty gap or missing piece in a pattern",
            ]
            non_target_prompts = [
                "a filled icon or complete image",
                "background texture",
                "text or header",
            ]
            ...
            pos_sim = (image_features @ pos_features.T).mean(dim=1)
            neg_sim = (image_features @ neg_features.T).mean(dim=1)
            scores = pos_sim - neg_sim
            ...
            print(f"DragSolver CLIP target: ({target_x:.0f}, {target_y:.0f}) [cell {best_row},{best_col}] score={scores[best_idx]:.3f}")
            return (target_x, target_y)
        except Exception as e:
            print(f"DragSolver CLIP error: {e}")
            return None

    async def _get_target_position_sift(self, frame, page: Page) -> Optional[Tuple[float, float]]:
        """
        Use SIFT + template matching as fast fallback.
        """
        try:
            print("DragSolver: Trying SIFT/template matching...")
            ...
            # Try to get the draggable icon image
            ...
            for scale in [0.5, 0.75, 1.0, 1.25]:
                sw = int(obj_gray.shape[1] * scale)
                sh = int(obj_gray.shape[0] * scale)
                if sw < 10 or sh < 10 or sw >= bg_gray.shape[1] or sh >= bg_gray.shape[0]:
                    continue
                scaled_obj = cv2.resize(obj_gray, (sw, sh))
                result = cv2.matchTemplate(bg_gray, scaled_obj, cv2.TM_CCOEFF_NORMED)
                _, max_val, _, max_loc = cv2.minMaxLoc(result)
                if max_val > best_val:
                    best_val = max_val
                    best_loc = (max_loc[0] + sw // 2, max_loc[1] + sh // 2)
            
            if best_loc and best_val > 0.3:
                print(f"DragSolver template: ({best_loc[0]}, {best_loc[1]}) conf={best_val:.3f}")
    ...
    async def solve(self, page: Page) -> bool:
        ...
        for round_num in range(self.config.max_challenge_rounds):
            ...
            # Determine target position - try SIFT first (instant), then CLIP, then CV
            target_pos = None
            
            print("DragSolver: Finding target position...")
            
            # Method 1: SIFT/template (instant, works if icon matches background)
            sift_target = await self._get_target_position_sift(frame, page)
            if sift_target:
                target_pos = sift_target
                print(f"DragSolver: Using SIFT result")
            
            # Method 2: CLIP (slower first time, but accurate for semantic matching)
            if not target_pos:
                clip_target = await self._get_target_position_clip(frame, page)
                if clip_target:
                    target_pos = clip_target
                    print(f"DragSolver: Using CLIP result")
            
            # Method 3: OpenCV contour analysis
            if not target_pos:
                cv_target = await self._get_target_position_cv(frame, page)
                if cv_target:
                    target_pos = cv_target
                    print(f"DragSolver: Using CV result")
            
            if not target_pos:
                print("DragSolver: All methods failed, using center estimate")
